...
void * ReceiveFromServer(void * arg)
{
	...
	prctl(PR_SET_NAME, (unsigned long)"ReceiveFromServer");
_Start:
	ret=InitTCP();
	if(ret == false)
		exit(0);
...
{
	prctl(PR_SET_NAME, (unsigned long)"HandleServerData");
	char data[FrameMaxLen];
	while(1)
	{
		if(IsQueueEmpty())
...
void * HeartBeat(void *arg)
{
	...
	char seq_buf[20];
	char sqlbuf[500];
	prctl(PR_SET_NAME, (unsigned long)"ServerHeartBeat");
	...
	sprintf(buf,"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
					"<Message>"
						...
						"<Body>"
							...
							"</Operation>"
						"</Body>"
					"</Message>",XmlVersion,Server.my_ip[0],Server.ip,seq_buf,"admin","123456");
...
void * Subscribe(void *arg)
{
	...
	prctl(PR_SET_NAME, (unsigned long)"SubscribeFromServer");
	int ret;
	int i,j;
...
{
	char xml_head[]="<?xml version=\"1.0\" encoding=\"utf-8\"?>";//<Message></Message>
	char xml_end[]="</Message>";
	if( ( memcmp(data,xml_head,strlen(xml_head) )== 0  )   && ( memcmp(data+strlen(data)-strlen(xml_end),xml_end,strlen(xml_end)) == 0 ))//&& ( memcmp(data+strlen(data)-strlen(xml_end),xml_end,strlen(xml_end)) == 0 )
	{
		return true;
	}
	else
